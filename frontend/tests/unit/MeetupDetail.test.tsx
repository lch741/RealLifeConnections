...
describe("Meetup Detail Page", () => {
  ...
  it("should render the meetup details header", () => {
    render(<MeetupDetailPage />);
    expect(screen.getByText(/^Meetup Details$/i)).toBeInTheDocument();
  });

  it("should show the meetup title", async () => {
    render(<MeetupDetailPage />);
    expect(
      await screen.findByRole("heading", { name: meetup.title, level: 1 }),
    ).toBeInTheDocument();
  });
});
